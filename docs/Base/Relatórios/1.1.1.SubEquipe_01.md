...
<!-- Comentário interno: Nova subseção adicionada para destacar o trabalho do Luiz Henrique -->
...
| Nome do Membro | Contribuição |
| --- | --- |
| Luiz Henrique Pallavicini | Ideação e elaboração do rascunho inicial do Diagrama de Sequência v1.0 |
...
### Ideação e Rascunho (Primeira Abordagem)

<!-- Comentário interno: Inserção do rascunho do Luiz antes do resultado final consolidado -->
A ideação primária do fluxo condicional (pedidos anônimos x não anônimos) foi desenhada estruturando o comportamento das requisições do sistema.

Artefato produzido:
[Rascunho Inicial do Diagrama de Sequência (Acesso no Figma)](https://www.figma.com/make/Qe1S4LmGmd7aP9wgsaUF54/Rascunho-de-Diagrama?t=ba2uUdZsLMrTomvI-20&fullscreen=1)
Autor: Luiz Henrique Pallavicini
...
| --- | --- | --- |
| Pedro Henrique Martins Silva | Aprendi um pouco mais - de forma superficial - sobre o funcionamento das soluções de softwares produzidas pelo gov em relação aos componentes por meio do chat realizado com o Gemini: https://share.google/aimode/nxj2ZtvHZF4n5bHGf que no fim produziu um diagrama básico de componentes do qual pude me inspirar ![Diagrama componentes v0.1](ExtrasSubEquipe_01/Diagrama_de_Componentes_V01.png) | Neste foco acabei usando a IA mais para melhorar a forma de expressar os passos que anotei e usei enquanto trabalhava nos diagramas, como exemplifica os dois primeiros prompts deste [chat com o GPT](https://chatgpt.com/share/6aaae424-b920-83e9-a357-b60b9698b3c2) e esta foto ![Exemplo](ExtrasSubEquipe_01/Exemplo_Uso_IA.png) e para mim neste uso ela foi muito boa, precisando apenas de alguns pequenos ajustes de semântica e coerência com o trabalho. Porém quando a tentei usar para ser revisora dos diagramas realizados ([como exemplifica esta foto](ExtrasSubEquipe_01/Exemplo_Uso_IA_2.png)) e para criar um cenário inicial para a diagramação estática senti que não foi tão útil, acredito que por não ter feito uma boa engenharia de prompt por conta da minha falta de domínio no assunto, um exemplo é no tempo que levei para me sentir minimamente satisfeito com os insumos que a IA havia entendido sobre o comportamento dos softwares do Governo para que aí sim permitisse que ela me gerasse um [diagrama de componentes](ExtrasSubEquipe_01/Diagrama_de_Componentes_V01.png) no qual eu pudesse me inspirar [(chat onde isto aconteceu)](https://share.google/aimode/nxj2ZtvHZF4n5bHGf). Tambpem usei a IA para configurar e estilizar o GitHub Pages e achei que foi de grande ajuda. |
| Luiz Henrique Pallavicini | Compreendi que a IA Generativa atua como uma excelente ferramenta de suporte e ideação, mas o esforço cognitivo principal e a tomada de decisão arquitetural devem permanecer com o desenvolvedor. O tempo economizado na validação de regras da UML e na geração de documentação permitiu focar mais na qualidade e clareza da entrega final. | Utilizei a IA em duas frentes: 1) Como suporte durante a ideação e modelagem no Figma, auxiliando no esclarecimento de regras específicas da UML para a v1.1 do Diagrama de Componentes e na validação do fluxo lógico do Diagrama de Sequência; 2) Como assistente de redação (*pair writing*) na estruturação do Markdown, revisão textual e padronização de commits. |
...
| Luiz Henrique Pallavicini | Ideação e Rascunho do Diagrama de Sequência v1.0 | 16/09 |
